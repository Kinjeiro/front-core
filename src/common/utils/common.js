...
export function formatString(str, ...args) {
  ...
  if (args.length) {
    ...
    strFinal = strFinal.replace(new RegExp('\\{(\\w+)\\}([ :\\-!]*)', 'g'), (match, key, space, position, allString) => {
      let value;
      if (paramArray) {
        value = isNaN(key)
          ? paramArray[paramArrayIndex++]
          : paramArray[key];
      } else {
        value = params[key];
      }
      ...
      if (itemFormatterFn) {
        value = itemFormatterFn(key, value, position, allString);
      }
