...
declare module "hapi" {
   ...
   *  boom provides a set of utilities for returning HTTP errors. Each utility returns a Boom error response object
   * (instance of Error) which includes the following properties:  */
  ...
  export interface IServerViewsEnginesOptions extends IServerViewsAdditionalOptions {
     ...
     * If the compileMode is 'sync', the signature is compile(template, options), the return value is a function with
     * signature function(context, options), and the method is allowed to throw errors.If the compileMode is 'async',
     * the signature is compile(template, options, callback) where callback has the signature function(err, compiled)
     * where compiled is a function with signature function(context, options, callback) and callback has the signature
  ...
  export interface IRouteConfiguration {
     ...
     * Can be assigned an array of methods which has the same result as adding the same route with different methods
     * manually.*/
  ...
  export class Server extends Events.EventEmitter {
     ...
     * When the server contains more than one connection, each server.connections array member provides its own
     * connection.info.
     var server = new Hapi.Server();
     server.connection({ port: 80 });
     // server.info.port === 80
